...
### THK-004 整个 turn 的账：10 步共 854 秒，其中一步占 75%
同一个 turn（owner 评价：结果做得很好，只是太久），取自 lmk 的 `LmkChatDone` 日志：
| 步 | prompt | 命中 | 写出 token | 首 token | 总时长 |
|---|---|---|---|---|---|
| .1 | 51,399 | 51,200 | 551 | 2.1s | 21.0s |
| **.3** | 64,619 | 51,200 | **15,698** | 62.8s | **645.6s** |
| .6 – .22（8 步） | 66k–75k | 每步差 0.2k–3k | 70–1,136 | 2.5–16.5s | 6–59s，合计 187.7s |
- 那一步写了 15,698 个 token（思考 + 工具调用的参数），单步 10 分 46 秒；其余 9 步合计 3 分 29 秒。
- cache 一直在起作用：75k 的 prompt，首 token 2–16 秒（长的那几次是在读 1.5k–3k token 的新工具结果）。
- 条件：`xhigh`（模板缺省）、贪心解码、M3 Ultra、无并发。质量评价来自 owner 的一句话，不是测量。
