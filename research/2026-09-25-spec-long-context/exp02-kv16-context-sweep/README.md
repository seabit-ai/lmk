...
## 结果（2026-09-25 21:27–21:50，`raw/`，全表 `results.md`）
关草稿 21:27–21:44（含 8k→128k 冷 prefill），开草稿 21:44–21:50（前缀命中关草稿时写下的 cache）。正式请求 60 个，全部 `length`、256 token；
常驻全程停着（每个请求的 `resident_poll_errors` > 0，作废 0 次）。前缀 token 数与 exp01 逐行相同（`raw/prefix.txt`）。
第一次启动因 `measure.py` 在关草稿时读 `draft.rounds` 取到 null 崩在第一个正式请求，修了（`.get`）清掉 raw 重跑；
`table.py` 的内存一节在跑完后把"kv8 推算"列换成了实测 peak（推算那列把 MLX 的缓冲池当成与 KV 无关，站不住），其余未改。

| context | kind | mode | 关：ms/步 kv8 → kv16 | 关 tok/s kv8 → kv16 | 开 tok/s kv8 → kv16 | 加速 kv8 → kv16 | 接受 kv8 → kv16 | 一轮 ms kv8（估）→ kv16（精确） |
|---|---|---|---|---|---|---|---|---|
| 8k | code | 贪心 | 28.3 → 27.0 | 35.3 → 37.0 | 53.0 → 53.7 | 1.50 → 1.45× | 73 → 74% | 71 → 70 |
| 8k | prose | 贪心 | 28.3 → 27.0 | 35.4 → 37.0 | 39.4 → 43.9 | 1.12 → 1.19× | 56 → 59% | 54 → 49 |
| 32k | code | 贪心 | 36.2 → 31.0 | 27.6 → 32.2 | 41.6 → 49.6 | 1.51 → 1.54× | 83 → 81% | 94 → 77 |
| 32k | prose | 贪心 | 36.1 → 31.1 | 27.7 → 32.2 | 35.3 → 37.7 | 1.27 → 1.17× | 63 → 63% | 64 → 60 |
| 64k | code | 贪心 | 46.3 → 35.9 | 21.6 → 27.8 | 30.9 → 41.6 | 1.43 → 1.49× | 81 → 79% | 123 → 90 |
| 64k | prose | 贪心 | 46.6 → 36.0 | 21.5 → 27.8 | 26.6 → 32.3 | 1.24 → 1.16× | 63 → 62% | 86 → 69 |
| 128k | code | 贪心 | 65.1 → 45.2 | 15.4 → 22.1 | 19.7 → 31.2 | 1.28 → 1.41× | 71 → 76% | 179 → 128 |
| 128k | prose | 贪心 | 65.6 → 45.2 | 15.2 → 22.1 | 18.8 → 24.0 | 1.23 → 1.08× | 63 → 59% | 120 → 94 |
| 32k | code | 采样 | 36.5 → 31.5 | 27.4 → 31.8 | 38.3 → 43.7 | 1.40 → 1.37× | 74 → 73% | 70 → 75 |
| 32k | prose | 采样 | 36.4 → 31.5 | 27.4 → 31.8 | 28.1 → 33.7 | 1.02 → 1.06× | 54 → 54% | 73 → 60 |

kv16 每格 3 次：关草稿离散 ≤ 1%；开草稿贪心 ≤ 12%（8k code 50.1–56.4），采样 prose 32.3–36.5、code 40.6–47.6。
精确轮数与 exp01 的估法（255 − 接受数）差 0–1 轮（如 8k prose 118 对 117），exp01 的"一轮 ms"可信。

内存（kv16 实测；kv8 的进程占用 exp01 没量）：

| context | KV 字节 kv8 → kv16（引擎 B/token × token） | kv16 进程持有（active + MLX 缓冲池）解码时最大，关 / 开 | kv16 MLX peak in use，关 / 开 |
|---|---|---|---|
| 8k | 0.30 → 0.56 GB | 18.9 / 20.9 GB | 20.4 / 19.1 GB |
| 32k | 1.15 → 2.17 GB | 25.5 / 27.7 GB | 24.6 / 23.9 GB |
| 64k | 2.29 → 4.32 GB | 34.2 / 36.7 GB | 30.1 / 30.4 GB |
| 128k | 4.57 → 8.61 GB | 52.0 / 54.2 GB | 42.2 / 43.3 GB |

262k：kv16 下引擎的 context-fit 仍是 `fitted=262,144`（`raw/context-fit-*.txt`：full_kv 65,536 B/token，working_set 77.76 GiB，baseline 14.95 GiB，
reserve 3 GiB），`lmk status` 窗口 262,144。按引擎公式，262k 需 262,144 × (65,536 + 10,240 + 48 × 2048) B ≈ 42.5 GiB，可用 59.8 GiB，**余约 17 GiB**
（kv8 同公式 35.0 GiB，余约 25 GiB）。代价在并发容量：`token_budget`（同时在内存里的 token 上限）1,844,474 → **979,877**。
262k 的请求没跑，这是公式，不是实测。

对预期：
- **P1 命中，而且比押的还多**：关草稿每步 27.0 / 31.0 / 35.9 / 45.2 ms（押 28 / 33 / 38 / 48–52），kv8 是 28.3 / 36.2 / 46.3 / 65.1。
  kv16 斜率 8k→128k 约 +0.15 ms / 1k，与 exp13 的 bf16 微基准一致；kv8 是 +0.30。带宽账（kv16 读的字节多该更慢）错了。
- P2 一半：开草稿确实跟着快（128k code 31.2 对 19.7 tok/s，是 kv8 开草稿的 1.58 倍），code 加速 1.41–1.54×；但 **prose 的加速比 kv8 小**
  （1.19 / 1.17 / 1.16 / 1.08×）：kv16 下 prose 一轮 ÷ 关的一步 = 1.81 / 1.93 / 1.92 / 2.08，比 kv8 的 1.8 左右高——底数（关草稿）快得更多。
- P3 命中：接受率 code 74–81%、prose 59–63%，与 kv8 同一范围。
- **P4 落空**：kv16 下 8 个贪心格子开/关文本也**全部分叉**（每边 3 次内部一致）。SLC-006 把分叉归到量化注意力，这一归因不成立——
  不量化也分叉，分叉来自批量校验（L>1）与单步（L=1）的浮点差本身。
- P5 命中（公式）：KV 字节与押的一致，262k 装得下，余约 17 GiB，token_budget 0.98M（押 0.9–1.0M）。进程实际持有比"权重 + KV"多得多
  （128k 52–54 GB）：MLX 的缓冲池留着 prefill / cache 还原时的临时缓冲；MLX peak in use 42–43 GB。
- 采样行：prose 1.06×（押 1.0–1.1）。**但"采样一轮贵 10 ms"在 kv16 下没有出现**：prose 采样一轮 60 ms、贪心 60 ms（精确轮数）；
  采样只因接受率低（54% 对 63%）多走轮数。exp01（kv8）是 73 对 64。这一点留给 exp03（kv8）去拆。
