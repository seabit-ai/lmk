...
### MG-007 准入队列把"读长 prompt 拖停别人"变成"后来者等前一个写完"（exp02）
同一场景：A 写 300 token 从 51.3 秒回到 8.2 秒；后来的 13k 冷 prompt 多等 8 秒（TTFT 43→51）。agent 的下一步
（27k prompt、27,136 命中）在别人写回答时不被排队，2.2 秒出字。队列在准入前对"未命中 token 数"的估计（读 store 的索引）
...
### MG-009 cache 命中的代价：取回 27k token 约 0.37 秒，11k 约 0.2 秒（直接量的，`restoreMs`）
`LmkChatDone.restoreMs` = 从把请求交给引擎、到引擎开始读未命中部分的时长。m3u，独占、cache 文件在系统页缓存里：
| 命中 | restoreMs | 首 token | 未命中要读的 |
|---|---|---|---|
| 27,136 / 27,190 | 360 / 360 / 372 / 367 ms | 765–810 ms | 54 |
| 11,008 / 11,139 | 216 / 186 ms | 857–888 ms | 131 |
两点近似一条线：约 75ms 固定 + 约 10ms / 1k token。对照：不用 cache 的 78 token 请求首 token 0.59–0.65 秒；27k 全部重算约 85 秒（320 tok/s）。
条件与未测：页缓存是热的（刚读过）；**重启机器后的真冷读没量**（09-19 服务重启后首句 2.4 秒是另一个条件）；27k 以上没量
（按这条线外推 100k ≈ 1.1 秒、262k ≈ 2.8 秒——外推，不是实测）；有别的请求在 decode 时首 token 升到约 2.1 秒。
别的代价：取回的 KV 照样占 GPU 内存（27k ≈ 1.7GB）；磁盘（现 21.9GB，约 5GB/天）；每个请求还要把新增的尾巴写盘（异步，不在关键路径上）。
