"""exp03 smoke: the engine's batched path with the MTP drafter — same prompts as exp01/02.

usage: smoke.py <out_dir>   (needs PYTHONPATH=<engine worktree>:<lmk>)
Runs each prompt greedy without and with the drafter (B=1), then two prompts at once (B=2).
"""
...
def main():
    ...
    stats0 = (kit._drafter.rounds, kit._drafter.accepted, kit._drafter.drafted)
    ...
    # sampling on (Qwen defaults) — acceptance only
    ...
    (out / "results.json").write_text(json.dumps(dict(base=base, spec=spec, sampled=s, pair=results, pair_wall_s=wall), indent=1, ensure_ascii=False))
