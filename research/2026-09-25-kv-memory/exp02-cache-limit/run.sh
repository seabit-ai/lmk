#!/bin/bash
# exp02: does capping MLX's buffer cache (mx.set_cache_limit before the engine loads) or clearing it when a
# request ends give back the ~10 GB without slowing decode? qwen3.8-27b-4bit, kv_cache_bits 16, dflash2 draft,
# 32k and 128k, code task, 3 x 256-token decodes per context, one temporary lmk per condition.
# usage: research/2026-09-25-kv-memory/exp02-cache-limit/run.sh
# Same harness as exp01 (measure.py / probe copied); the limit is set by probe/sitecustomize.py from
...
CONDS="${CONDS:-unset1:: limit0:0: limit1g:1073741824: limit4g:4294967296: clear::1 unset2::}"
PORT=1236
WORK="$(mktemp -d /tmp/lmk-kvm2.XXXXXX)"   # one prompt cache shared by all conditions: only the first prefills cold
PID=""
...
model: {name: qwen3.8-27b-4bit, kv_cache_bits: 16, thinking: false, speculative_decoding: true, draft: dflash2}
listen: {port: $PORT}
