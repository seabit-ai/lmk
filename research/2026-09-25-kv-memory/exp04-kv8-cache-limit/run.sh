#!/bin/bash
# exp04: exp02 at kv_cache_bits 8 — does a 1 GiB MLX cache limit give the buffer pool back without slowing the kv8 decode?
# qwen3.8-27b-4bit, dflash2 draft, 32k and 128k, code task, 3 x 256-token decodes per context, one temporary lmk per condition.
# usage: research/2026-09-25-kv-memory/exp04-kv8-cache-limit/run.sh
...
CONDS="${CONDS:-unset1:: limit1g:1073741824: unset2::}"
PORT=1236
WORK="$(mktemp -d /tmp/lmk-kvm4.XXXXXX)"   # one prompt cache shared by all conditions: only the first prefills cold
PID=""
