...
- **预期 3 部分押中**：思考确实帮格式题——27B-off 的七次失误是 yes-or-no ×3（答 "**No**" 加粗、判分器按纯词核对，见下）、
  no-letter-e ×3、five-words ×1；low 和 xhigh 把这些都补上了。
- 判分口径：yes-or-no 的三次失误里答案本身是对的（No），格式带了 markdown 加粗；按题面"exactly one word"仍算错——记下，
  汇总时列进"判分边界"。
