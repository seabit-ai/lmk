...
t0=$(date +%s)
.venv/bin/python -P -m lmk serve > "$HERE/raw/serve.log" 2>&1 &
PID=$!
...
curl -s -m 600 http://127.0.0.1:1236/v1/chat/completions -H 'content-type: application/json' \
  -d "{\"model\":\"probe\",\"messages\":[{\"role\":\"user\",\"content\":\"$LONG Reply with the single word: done\"}],\"temperature\":0,\"max_tokens\":32}" > "$HERE/raw/cold.json"
curl -s -m 600 http://127.0.0.1:1236/v1/chat/completions -H 'content-type: application/json' \
  -d '{"model":"probe","messages":[{"role":"user","content":"Write a 250-word story about a lighthouse keeper."}],"temperature":0,"max_tokens":400}' > "$HERE/raw/decode.json"
kill $PID; wait $PID 2>/dev/null || true
cp "$H/logs/lmk.jsonl" "$HERE/raw/lmk.jsonl"
python3 - "$HERE/raw/lmk.jsonl" <<'PY'
import json, sys
